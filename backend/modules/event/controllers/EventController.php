...
class EventController extends Controller
{
    ...
    public function actionInvitcreate($rg_id)
    {
        ...
        if ($invition->load(Yii::$app->request->post())) {
            ...
            if($invition->save()){
                ...
                ->useLogo(Yii::$app->basePath. '/web/images/qrcodeogo.png')
                ->setSize(250)
                ->setMargin(5);
    ...
    public function actionRegisterdit($id,$reg_id)
    {
        $model = Registration::find()->where(['id'=>$id])->one();
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['listregistered', 'id' =>$reg_id]);
        } else {
            return $this->render('register_edit',['model'=>$model,]);
        }
    }
    public function actionRegistdel($id,$reg_id)
    {
        $model = Registration::find()->where(['id'=>$id])->one();
        if(!empty($model))
        {
